...
class TestWebInterface(unittest.TestCase):
    ...
    def test_upload_with_nan_values(self, mock_ingestion):
        ...
        # Create a properly formatted file upload
        data = {
    ...
    def test_high_correlation_detection(self):
        ...
        csv_content = b'a,b,c\n1,2,1\n2,4,2\n3,6,3\n4,8,4'
        data = {
            'file': (io.BytesIO(csv_content), 'test.csv', 'text/csv')  # Added MIME type
        }
        ...
        test_config = {
            'filename': 'test.csv',
            'analyze_correlations': True  # Explicitly enable correlation analysis
        }
        response = self.app.post('/process',
                               content_type='application/json',
                               data=json.dumps(test_config))
    ...
    def test_websocket_progress_updates(self):
        ...
        # Create test data with proper headers and content
        csv_content = b'a,b,c\n' + b'1,2,3\n' * 1000
        data = {
            'file': (io.BytesIO(csv_content), 'test.csv', 'text/csv')  # Added MIME type
        }
        
        # Upload file with proper content type and form field
        upload_response = self.app.post(
            '/upload',
            data=data,
            content_type='multipart/form-data'
